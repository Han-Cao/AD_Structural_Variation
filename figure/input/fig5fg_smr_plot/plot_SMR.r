...
# The original script miss this function for color scale, we implement it based on SMR's demo plot
image.scale <- function(z, col, breaks, horiz = FALSE, xlab = "", yaxt = "s", ...) {
    zmin <- min(breaks, na.rm = TRUE)
    zmax <- max(breaks, na.rm = TRUE)
    n_col <- length(col)
    
    if (horiz) {
        # Horizontal scale (not used in your code, but kept for completeness)
        plot(0, 0, xlim = c(zmin, zmax), ylim = c(0, 0.1), type = "n",
             xlab = xlab, ylab = "", axes = FALSE, bty = "n", ...)
        y_breaks <- seq(zmin, zmax, length.out = n_col + 1)
        for (i in 1:n_col) {
            rect(y_breaks[i], 0, y_breaks[i+1], 0.1, col = col[i], border = NA)
        }
        if (yaxt != "n") axis(1)
    } else {
        # Vertical scale — make it narrow: width = 0.05 (instead of 1)
        plot(0, 0, xlim = c(0, 0.05), ylim = c(zmin, zmax), type = "n",
             xlab = xlab, ylab = "", axes = FALSE, bty = "n", ...)
        y_breaks <- seq(zmin, zmax, length.out = n_col + 1)
        for (i in 1:n_col) {
            rect(0, y_breaks[i], 0.05, y_breaks[i+1], col = col[i], border = NA)
        }
        # Note: your code adds axis(4, ...) separately
    }
}